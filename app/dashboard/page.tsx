...
export default function DashboardPage() {
  ...
  async function loadDashboard() {
    try {
      ...
      let userWithName = { ...currentUser };
      
      // Jika user tidak punya name, coba ambil dari localStorage
      if (!userWithName.name || userWithName.name === "Pengguna") {
        const savedName = localStorage.getItem("registerName") || 
                          localStorage.getItem("userName");
        
        if (savedName) {
          userWithName.name = savedName;
          console.log("✅ Nama diambil dari localStorage:", savedName);
          
          // Update user di localStorage
          localStorage.setItem("user", JSON.stringify(userWithName));
        } else {
          // Fallback: ambil dari email
          const emailName = currentUser.email?.split("@")[0];
          if (emailName) {
            userWithName.name = emailName.charAt(0).toUpperCase() + emailName.slice(1);
            localStorage.setItem("user", JSON.stringify(userWithName));
          }
        }
      }
      ...
      const currentEmail = userWithName?.email?.toLowerCase().trim();

      if (!currentEmail) {
        throw new Error("Email user tidak ditemukan.");
      }

      console.log("USER LOGIN:", userWithName);
      ...
      const response = await apiFetch("/tickets?page=1&limit=100");
      console.log("SEMUA TIKET DARI BACKEND:", response);
      ...
      const userTickets = allTickets.filter((ticket) => {
        ...
        return ticketEmail.toLowerCase().trim() === currentEmail;
      });

      console.log("TIKET USER INI:", userTickets);
      ...
      const processing = userTickets.filter(
        (ticket) => ticket.status?.toLowerCase() === "in_progress"
      ).length;
      const completed = userTickets.filter((ticket) => {
  ...
  function getTicketNumber(ticket: TicketData) {
    return ticket.ticketNumber || ticket.ticket_number || `#${ticket.id}`;
  }
  ...
  function getTicketTitle(ticket: TicketData) {
    return ticket.subject || ticket.title || "Tanpa judul";
  }
  ...
  function getStatusLabel(status?: string) {
    ...
    switch (normalized) {
      ...
      case "waiting_reply": return "Menunggu Balasan";
      case "escalated": return "Diteruskan";
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm text-slate-500 mt-4">Memuat dashboard...</p>
        </div>
      </div>
    );
  }
            ...
            <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-5">
              <div>
                ...
                <h1 className="text-3xl font-bold tracking-tight text-slate-900">
                  Selamat Datang
                  {user?.name ? `, ${user.name}` : ""}
                  {!user?.name && <span className="text-slate-400">, Pengguna</span>}
                </h1>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-1">Tiket milik Anda</p>
                </div>
                <div className="w-11 h-11 rounded-lg bg-blue-50 flex items-center justify-center">
                  <Ticket size={21} className="text-blue-600" />
                </div>
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-1">Sedang ditangani</p>
                </div>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
              <div className="flex items-center justify-between">
                ...
                <div className="w-11 h-11 rounded-lg bg-slate-100 flex items-center justify-center">
                  <CheckCircle2 size={21} className="text-slate-600" />
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              >
                ...
                <h3 className="text-base font-bold mt-5">Buat Tiket</h3>
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="w-10 h-10 rounded-lg bg-slate-100 flex items-center justify-center">
                    <Star size={21} className="text-slate-600" />
                  </div>
                  <ArrowRight
                    size={18}
                      ...
                      <td colSpan={4} className="px-5 py-12 text-center">
                        <div className="flex flex-col items-center">
                          <div className="w-12 h-12 rounded-full bg-slate-100 flex items-center justify-center">
                            <Ticket size={21} className="text-slate-400" />
                          </div>
